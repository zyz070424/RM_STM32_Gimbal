 ...
 * @return 限幅后的值。
 */
float Class_Gimbal_Sentry_Target::Clamp(float value, float min_value, float max_value) const
{
    if (value < min_value)
    {
        return min_value;
 ...
 * @return 滤波后的输出值。
 */
float Class_Gimbal_Sentry_Target::FirstOrderLowPass(float current_value,
                                                    float target_value,
                                                    float tau_s,
                                                    float dt_s) const
{
...
void Class_Gimbal_Sentry_Target::UpdateFilter(uint8_t target_available)
{
    ...
    visual_filtered_pitch_deg = FirstOrderLowPass(visual_filtered_pitch_deg,
                                                  visual_last_valid_pitch_deg,
                                                  config.vision_target_filter_tau_s,
                                                  config.control_dt_s);
    visual_filtered_yaw_deg = FirstOrderLowPass(visual_filtered_yaw_deg,
                                                visual_last_valid_yaw_deg,
                                                config.vision_target_filter_tau_s,
                                                config.control_dt_s);
}
